package net.baphy.schematicon.client;

import net.baphy.schematicon.item.CellBrushItem;
import net.baphy.schematicon.network.ModPackets;
...
public class CellBrushClientHandler {
    ...
    public static void onClientTick(Minecraft mc) {
        ...
        if (!mc.level.dimension().equals(ModDimensions.SCHEMATICON_KEY)){
    ...
    public static boolean onMouseScroll(double delta) {
        ...
        if (!mc.level.dimension().equals(ModDimensions.SCHEMATICON_KEY)) return false;
    ...
    public static void onLeftClick() {
        Minecraft mc = Minecraft.getInstance();
        if (mc.player == null || mc.level == null) return;
        if (!mc.level.dimension().equals(ModDimensions.SCHEMATICON_KEY)) return;
    ...
    public static void applySelection(Player player) {
        ...
        if (clientSelection.isEmpty()) {
            player.displayClientMessage(Component.translatable("item.schematicon.cell_brush.nothing_selected").withColor(0xFF0000), true);
            return;
        }
